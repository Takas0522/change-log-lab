...
| 要件ID | 説明 | 目標値 | 測定条件 |
|---|---|---|---|
| REQ-PERF-001 | API応答時間（単一ToDo操作） | 500ms以内 | 通常負荷時、95パーセンタイル |
| REQ-PERF-002 | API応答時間（一覧取得） | 1秒以内 | 1000件以下、通常負荷時、95パーセンタイル |
| REQ-PERF-003 | 画面初期表示時間 | 2秒以内 | 初回アクセス、通常ネットワーク環境 |
| REQ-PERF-004 | 画面遷移時間 | 500ms以内 | SPA内遷移、キャッシュ有効時 |
| REQ-PERF-005 | 同時接続ユーザー数 | 100ユーザー | レスポンスタイム劣化なし |
| REQ-PERF-006 | データベーステーブルサイズ | 100万レコード | 性能劣化なし |
| REQ-PERF-007 | 検索処理時間 | 1秒以内 | 全文検索、10万件以下 |

#### 3.2.2 ユーザビリティ要件

| 要件ID | 説明 | 基準 |
|---|---|---|
| REQ-USE-001 | レスポンシブデザイン | デスクトップ、タブレット、スマートフォンで使用可能 |
| REQ-USE-002 | ブラウザ対応 | Chrome（最新版）、Edge（最新版）、Firefox（最新版）、Safari（最新版） |
| REQ-USE-003 | 画面解像度 | 最小1280x720、推奨1920x1080 |
| REQ-USE-004 | 操作の直感性 | 初回利用時にマニュアル不要で基本操作が可能 |
| REQ-USE-005 | エラーメッセージ | ユーザーフレンドリーで具体的な対処法を提示 |
| REQ-USE-006 | 入力補助 | 必須項目の明示、入力形式のヒント表示 |
| REQ-USE-007 | キーボードショートカット | ToDo作成（Ctrl+N）、検索（Ctrl+F）等の提供 |
| REQ-USE-008 | アクセシビリティ | WCAG 2.1 Level AA準拠 |
...
| 要件ID | 説明 | カテゴリ |
|---|---|---|
| REQ-SEC-001 | SQLインジェクション対策 | パラメータ化クエリの使用 | データ保護 |
| REQ-SEC-002 | XSS対策 | 入力値のサニタイゼーション、出力エスケープ | データ保護 |
| REQ-SEC-003 | CSRF対策 | アンチCSRFトークンの使用 | データ保護 |
| REQ-SEC-004 | HTTPS通信 | すべての通信をHTTPS化 | 通信保護 |
| REQ-SEC-005 | 入力値検証 | サーバー側での厳密なバリデーション | データ保護 |
| REQ-SEC-006 | APIレート制限 | 過度なリクエストの制限 | 可用性保護 |
...
| Status | VARCHAR(20) | NOT NULL, DEFAULT '未着手' | ステータス（未着手/着手中/完了/放棄） |
| CreatedAt | DATETIME | NOT NULL, DEFAULT CURRENT_TIMESTAMP | 投稿日時 |
...
| 項目名 | データ型 | 制約 | 説明 |
|---|---|---|---|
| ToDoID | INT / BIGINT | PK, FK → ToDo.ToDoID | ToDo識別子 |
| LabelID | INT | PK, FK → Label.LabelID | ラベル識別子 |
| CreatedAt | DATETIME | NOT NULL, DEFAULT CURRENT_TIMESTAMP | 紐付け日時 |
