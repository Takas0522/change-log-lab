...
argument-hint: phase={requirements|design|development|testing}, 成果物パス, PR番号
...
### 5. PRコメントへの記録
```bash
gh pr comment <pr-number> --body "<レビュー結果>"
```
...
### コメント投稿
```bash
gh pr comment <pr-number> --body "## [フェーズ]レビュー結果
...
### リトライ時のコメント
```bash
gh pr comment <pr-number> --body "## [フェーズ]レビュー結果 (リトライ X/3)
